...
**Exports:**
- `async fn search_users()` - GET /v1/users/search - Search users by callsign (public)
- `async fn register()` - POST /v1/register - Register user and get auth token
- `async fn delete_account()` - DELETE /v1/account - Delete own account (auth required)
- `async fn update_callsign()` - PUT /v1/account/callsign - Update callsign across all data (auth required)
